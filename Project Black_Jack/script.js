...
var main = function (input) {
  ...
  if (gameMode === GAME_START) {
    ...
    // Deal 2 two cards to player 1 and dealer // pop a for loop(shorten)
    ...
    button.innerText;
    button.innerText = "Check Cards";
    button.style.color = "Green"

    // Return Output Message
    outputMessage = "Cards are Dealts(Please Check Your Cards)";
    ...
    if (playerOneHasBlackJack === true || dealerHasBlackJack === true) {
      ...
      if (playerOneHasBlackJack === true && dealerHasBlackJack === true) {
        outputMessage =
          ...
          "<b>It is a Blackjack tie! Neither Comp or Player Wins!</b> <br><br> Refresh the page to play again.";
      }
      ...
      if (playerOneHasBlackJack === false && dealerHasBlackJack === true) {
        outputMessage =
          displayHand(playerOneDeck, dealerHand) +
          "<b>Computer wins with Blackjack! <br><br> Refresh the page to play again.";
      }
    } else {
      outputMessage =
        displayHand(playerOneDeck, dealerHand) +
  ...
  if (gameMode === GAME_HIT_OR_STAND) {
    ...
    if (input === "hit") {
      ...
      if (totalPlayerHandValue > 21) {
        console.log(totalPlayerHandValue)
        gameResults = true;
        outputMessage =
          displayHand(playerOneDeck, dealerHand) +
          "<b>It's a bust! better luck next time</b> You lose. <br><br> Refresh the page to play again.";
      } else
        outputMessage =
          displayHand(playerOneDeck, dealerHand) +
      ...
      if (totalPlayerHandValue === totalDealerHandValue) {
        outputMessage =
          displayHand(playerOneDeck, dealerHand) +
          "<b>Its a tie! neither computer or player won!</b> <br><br> Refresh the page to play again.";
      }
      ...
      // dealer higher value hand --> dealer win
        ...
        outputMessage =
          displayHand(playerOneDeck, dealerHand) +
          "<b>Computer wins!</b> <br><br> Refresh the page to play again.";
      }
    } else
      outputMessage =
...
var calculateHand = function (handArray) {
  ...
  while (index < handArray.length) {
    ...
    if (
      ...
      totalHandValue = totalHandValue + 10; // give J, Q, K weight of 10
    } else {
      totalHandValue = totalHandValue + currentCard.number; // set the weight of cards.number
    }
    index += 1;
  }
...
var displayHand = function (playerHandArray, dealerHandArray) {
  ...
  var index = 0;
  if (gameResults === true) { // if game results true it will just display the final results(regardless)
    ...
    var dealerMessage = "Computer hand: <br> ";
    index = 0;
    ...
    finalMessage =
      ...
      totalDealerScore +
      "<br><br>";
  } else { // this is game results = false continue to keep the gameplay going
    while (index < playerHandArray.length) {
      playerMessage =
        playerMessage +
        "- " +
    ...
    var dealerMessage = "Computer hand: <br> - ? <br>";
    index = 1;
    while (index < dealerHandArray.length) {
      dealerMessage =
        dealerMessage +
